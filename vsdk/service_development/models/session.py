...
class CallSession(models.Model):
    ...
    user = models.ForeignKey(KasaDakaUser, on_delete=models.SET_NULL, null=True,
                             blank=True)
    ...
    def __str__(self):
        ...
        if self.user:
            ...
            return "%s (%s %s)" % (
            str(self.caller_id), str(start_date), str(start_time))
...
class CallSessionStep(models.Model):
    ...
    session = models.ForeignKey(CallSession, on_delete=models.CASCADE,
                                related_name="steps")
    _visited_element = models.ForeignKey(VoiceServiceElement, on_delete=models.SET_NULL,
                                         null=True)
    ...
    def __str__(self):
        ...
        if self.visited_element:
            return "%s: @ %s -> %s" % (
            str(self.session), str(datetime), str(self.visited_element))
        elif self.description:
            return "%s: @ %s -> %s" % (
            str(self.session), str(datetime), str(self.description))
        else:
